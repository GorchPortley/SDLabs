...
<div x-data="{ selectedTab: 'summary' }" class="w-full min-h-screen">
    ...
    <div class="px-2 py-4 min-h-full text-neutral-600 dark:text-neutral-300">
        <div x-show="selectedTab === 'summary'" id="tabpanelSummary" class="h-auto min-h-full w-full" role="tabpanel" aria-label="summary">
            <div>
            <livewire:frequency-response-viewer :design="$design" />
            <iframe class="w-full h-auto" srcdoc="{{ $design->summary }}" seamless sandbox></iframe>
        </div>
        <div class="w-full h-auto min-h-full" x-show="selectedTab === 'description'" id="tabpanelDescription" role="tabpanel" aria-label="description">
            @if($this->hasAccess)
                <div>
                    <div class="w-full">
                        <iframe class="w-full h-auto" srcdoc="{{ $design->description }}" seamless sandbox></iframe>
                    </div>
                </div>
            @else
        ...
        <div class="w-full h-auto min-h-full overflow-auto" x-show="selectedTab === 'discussion'" id="tabpanelDiscussion" role="tabpanel" aria-label="discussion">
            @if($this->hasAccess)
                <div>
                    <div class="w-full ">
        ...
        <div x-show="selectedTab === 'components'" id="tabpanelcomponents" role="tabpanel" class="w-full h-auto min-h-full" aria-label="components">
            @if($this->hasAccess)
                <div>
                    <div class="w-full h-auto min-h-full overflow-auto">
                        @foreach($design->components as $component)
                            <div x-data="{ isOpen: false }" class="border-t border-gray-200 pt-8">
                                ...
                                >
                                    <iframe class="w-full h-auto" srcdoc="{{$component->description}}" sandbox seamless></iframe>
                                </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="" x-show="selectedTab === 'files'" id="tabpanelFiles" role="tabpanel" aria-label="files">
            ...
            @if($this->hasAccess)
                <div>
                    <div class="w-full h-auto min-h-full">
                        <iframe class="w-full h-auto" src="/filemanager"></iframe>
                    </div>
                </div>
            @else
                <div class="w-full bg-zinc-600 p-8 rounded-lg text-center">
